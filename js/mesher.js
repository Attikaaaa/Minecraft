...
const pushQuad = (buffers, origin, du, dv, normal, tile, flip, isWater, worldPos) => {
  ...
  // Víznél world koordinátákat használunk a folyamatos textúrához
  let u0 = 0, v0val = 0, u1 = uMax, v1val = 0, u2 = uMax, v2val = vMax, u3 = 0, v3val = vMax;
  
  if (isWater && worldPos) {
    // World koordináták alapján számoljuk a UV-t
    const [wx, wy, wz] = worldPos;
    
    // Melyik tengely mentén van a quad?
    if (Math.abs(nx) > 0) {
      // X tengely (YZ sík)
      u0 = wz + v0[2] - oz;
      v0val = wy + v0[1] - oy;
